...
For Codex, the agent runs `codex mcp login sensai` and waits for the command's actual completion. It does not open, copy, or request an authorization URL, code, or token manually. For Claude Code, the agent runs `claude mcp login plugin:sensai:sensai`, wrapped so the command receives a terminal on its standard input: `script` on macOS and Linux, a minimized console on Windows. An agent's shell provides no terminal, and the bare command stops with `stdin isn't a terminal`. The exact wrappers are in the README.
